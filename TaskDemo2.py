...
def save_terrain_image(height_map, filename='perlin_terrainDemo.png'):
    plt.imsave(filename, height_map, cmap='terrain')

if __name__ == "__main__":
    # Increase the resolution for larger maps
    resolution = (1024, 2048)  # Larger resolution
    base = 1.8
    number = 10
    offset = 1
    water = 6

    # Generate and visualize multiple terrains
    num_maps = 3  # Number of maps to generate
    terrains = []

    for i in range(num_maps):
        np.random.seed(i)  # Different seed for variability
        terrain = island(resolution[1], resolution[0], frequency=0.1, amplitude=1.0, gradient=True)
        terrains.append(terrain)
        save_terrain_image(terrain, filename=f'perlin_terrain_{i}.png')
        print(f"Generated terrain {i}")

    # Visualize using blueprints (select one to visualize)
    # For demonstration, visualize the first one
    visualize_with_blueprints(terrains[0])

    # Select the most interesting map manually by inspecting the saved images
    # For now, let's assume the first one is selected
    selected_terrain = terrains[0]
    save_terrain_image(selected_terrain, filename='selected_perlin_terrain.png')
